...
import Forecast_Panel from "@/components/Forecast_Panel";
...
type forecast_info_type = [
  ...
  setForecast_info : Dispatch<SetStateAction<{[key: string] : string | number | Array<object>}>>
]
...
export default function Home() {
  ...
  const getWeather = () => {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY}&units=metric&lang=pl`)
      .then(res => res.json())
      .then(res => setWeather_info(res))
      .catch(res => console.warn('Zapytanie zawiodło'));
    fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY}&units=metric&lang=pl`)
      .then(res => res.json())
  ...
  return (
    <div className="grid grid-cols-4 grow gap-0">
      <div className="flex flex-col flex-wrap col-span-1 p-3 bg-gray-400 gap-2">
        <Input
        id="city_input"
        type="text"
        className="w-5/6"
        ...
        <Button 
        className="w-1/3 place-self-end"
        onClick={getWeather}
        >
          Wyszukaj
        </Button>
      </div>
      <div className="flex flex-col flex-wrap grow col-span-3 bg-gray-200">
        {
          Object.keys(weather_info).length !== 0 && Object.keys(forecast_info).length !== 0
          ?
          <>
          <Weather_Panel weather_info={weather_info} />
          <Forecast_Panel forecast_info={forecast_info} />
          </>
          :
          <></>
        }
